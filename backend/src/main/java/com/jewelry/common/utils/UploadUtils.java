...
public class UploadUtils {
    public static String storeImage(MultipartFile imageFile, String path)  {
        try {
            if (imageFile.getSize() == 0) {
                throw new BaseDomainException( "Please select images to upload");
            }
            if (!isImageFile(imageFile) || imageFile.getOriginalFilename() == null) {
                throw new BaseDomainException("Invalid image format");
            }
            if (imageFile.getSize() > 3 * 1024 * 1024) {
                throw new BaseDomainException("File must be <= 5MB");
            }
    ...
    public static String storeVideo(MultipartFile videoFile, String path) {
        ...
        catch (BaseDomainException appException){
            throw appException;
        }
        catch (Exception ex) {
            throw new BaseDomainException( "Lưu video bị lỗi");
        }
    }
