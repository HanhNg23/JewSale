...
function CapNhat() {
  ...
  const [reponseOrder] = useFetchV2(
    { fetchFunction: () => axios.get(`http://localhost:8080/invoices/${id}`) },
    id
  );
  ...
  const setParams = (params) => {
    ...
    if (params.searchKeyword) {
      ...
      queryParams.delete("searchKeyword");
    }
    if (params.productType) {
      params.productType.forEach((e) => {
        console.log(e);
        queryParams.append("productType", e.toString());
      });
      console.log(queryParams.toString());
    } else {
      queryParams.delete("productType");
    }
  ...
  const columns = [
      ...
      render: (_, { imageUrls }) => {
        ...
        return (
          <div style={{ minHeight: 100 }}>
            <Image
              width={100}
              src={`http://localhost:8080/uploads/image/${imageUrls[0]?.url}`}
    ...
    },
    {
      ...
      key: "productPrice",
      render: (_, { productPrice }) => {
        return formatCurrencyVND(productPrice?.salePrice);
      },
    },
    {
      title: "Tình trạng",
      ...
      render: (text) => {
        ...
        if (text === "hết hàng") {
          return (
            <Typography.Text style={{ color: "#A18210" }}>
              hết hàng
            </Typography.Text>
          );
        }
        if (text === "ngừng bán") {
          return (
            <Typography.Text style={{ color: "#ED3237" }}>
              Ngừng bán
            </Typography.Text>
          );
        }
  ...
  const cart_columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button onClick={() => handleIncreaseQuantity(record.productId)}>
            +
          </Button>
          {record.quantity}
          <Button onClick={() => handleDecreaseQuantity(record.productId)}>
            -
          </Button>
        </Space>
      ),
    },
    {
      ...
      key: "salePrice",
      render: (_, record) => {
        return formatCurrencyVND(record.productPrice.salePrice);
      },
    },
    {
      title: "Thành tiền",
  ...
  const handleRemoveItem = (productId) => {
    setCart((prevCart) =>
      prevCart.filter((cartItem) => cartItem.productId !== productId)
    );
  };
      ...
      prevCart.map((cartItem) =>
        cartItem.productId === productId
          ? cartItem.quantity < cartItem.stockQuantity
            ? { ...cartItem, quantity: cartItem.quantity + 1 }
            : cartItem // If quantity >= stockQuantity, do not increase
          : cartItem
      )
    );
  };
  ...
  const getTotalPrice = () => {
    return cart.reduce(
      (total, item) => total + item.productPrice.salePrice * item.quantity,
      0
    );
  };
  const onFinish = async (values) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8080/invoices/invoice/${id}`, payload, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
      notification.success({ message: "Cập nhật đơn thành công" });
      setCart([]);
      setPayment("CASH");
  ...
  useEffect(() => {
    if (reponseOrder) {
      ...
      sellForm.setFieldsValue({
        customerName: reponseOrder.customer.fullname,
        customerPhone: reponseOrder.customer.phonenumber,
      });
      setPayment(reponseOrder.paymentMethod);
      setCart(
        reponseOrder.invoiceItems.map((i) => ({
          ...i.product,
          quantity: i.quantity,
        }))
      );
          ...
          <Row gutter={[16, 16]} align={"middle"}>
            <Col span={16}>
              <Row gutter={[16, 16]}>
                ...
                <Col span={8}>
                  {reponseOrder &&
                    reponseOrder.status === "PENDING_PAYMENT" && (
                      <Tag
                        color="warning"
                  ...
                  {reponseOrder && reponseOrder.status === "PAID" && (
                    <Tag
                      color="success"
                      style={{ height: 40, lineHeight: "40px" }}
                    >
                  ...
                  {reponseOrder && reponseOrder.status === "CANCELLED" && (
                    <Tag
                      color="error"
                      style={{ height: 40, lineHeight: "40px" }}
                    >
              ...
              children: (
                <div>
                  ...
                  >
                    <Row gutter={[16, 16]}>
                      <Col span={8}>
                        <Form.Item label="Mã ID sản phẩm" name="username">
                          <Input size="large" />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                ...
                <p className="brown-paragraph">
                  ...
                  <Table
                    columns={columns}
                    pagination={{
                      current: searchParams.get("pageNo")
                        ? Number(searchParams.get("pageNo"))
                        : 1,
                      onChange: (page, pageSize) => {
                        console.log(page, pageSize);
                        setParams({ pageNo: page, pageSize: pageSize });
                      },
                      total:
